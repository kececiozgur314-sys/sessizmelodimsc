...
@app.on_message(filters.command(["broadcast"]) & ~app.bl_users)
@lang.language()
async def _broadcast(_, message: types.Message):
    global broadcasting
    if message.from_user.id not in app.sudoers:
        return await message.reply_text(
            message.lang.get("sudo_only", "This command is only for sudo users.")
        )
...
@app.on_message(filters.command(["stop_gcast", "stop_broadcast"]) & ~app.bl_users)
@lang.language()
