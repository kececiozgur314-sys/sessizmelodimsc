...
async def _watcher_vc(_, m: types.Message):
    ...
    if getattr(m, "video_chat_started", None):
        text = _lang.get("voice_chat_started", "🔔 Voice chat has started!")
    else:
        text = _lang.get("voice_chat_ended", "🔕 Voice chat ended.")
    try:
...
@app.on_message(filters.video_chat_members_invited, group=21)
@lang.language()
async def _vc_invited(_, m: types.Message):
    inviter = m.from_user.mention if m.from_user else m.lang.get("unknown_user", "Unknown")
    invited = getattr(m, "video_chat_members_invited", None)
    if not invited or not invited.users:
        return
    mentions = ", ".join([user.mention for user in invited.users])
    text = m.lang.get(
        "voice_chat_invited",
        "{inviter} invited {invited} to the voice chat.",
    ).format(inviter=inviter, invited=mentions)
    try:
        await app.send_message(chat_id=m.chat.id, text=text)
    except Exception:
        pass
